...
class LanguageCrudController extends CrudController
{
    ...
    public function setupCreateOperation()
    {
        ...
        if($this->crud->getOperation() == 'update' && $this->crud->getCurrentEntry()->default == 0) {
            $this->crud->addField([
                'name' => 'default',
                'label' => trans('backpack::langfilemanager.default'),
                'type' => 'checkbox',
    ...
    public function store()
    {
        ...
        $storeResult = $this->traitStore();

        if(request()->get('default') == 1) {
            Language::where('id', '!=', $this->crud->getCurrentEntry()->id)->update(['default'=>0]);
        }
    ...
    {
        if(request()->get('default') == 1) {
            Language::where('id', '!=', request()->get('id'))->update(['default'=>0]);
        }
