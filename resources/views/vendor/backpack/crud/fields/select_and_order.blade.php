...
<div class="row">
    ...
    <div class="col-md-6">
        <label>{!! $field['label_selected'] !!}</label>
    </div>
</div>
<div class="row"
    ...
    <div class="col-md-12">
        <ul data-identifier="drag-source"
            class="{{ $field['name'] }}_connectedSortable select_and_order_all float-left"></ul>
        <ul data-identifier="drag-destination"
            class="{{ $field['name'] }}_connectedSortable select_and_order_selected float-right"></ul>
    ...
    @if(isset($field['select_all']) && $field['select_all'])
        <div class="col-md-12">
            <div class="array-controls btn-group m-t-10">
                <a class="btn btn-xs btn-default select_all" onclick="selectAll()"><i
                            class="la la-check-square-o"></i> {{ trans('backpack::crud.select_all') }}</a>
            </div>
            <div class="array-controls btn-group m-t-10">
                <a class="btn btn-xs btn-default clear" onclick="clearAll()"><i
...
@if ($crud->fieldTypeNotLoaded($field))
    ...
    <script>
        ...
        function clearAll() {
            theField = $('[data-field-name={{ $field['name'] }}]')

            var $dragSource = theField.find('[data-identifier=drag-source]')
            var $dragDestination = theField.find('[data-identifier=drag-destination]')
            var $hiddenSelect = theField.find('[data-identifier=results] select')

            $(theField).find('[data-identifier=drag-destination] li').each(function (e) {
                itemValue = $(this).val()
                itemText = $(this).text()

                // Copy the item in the destination area
                $dragSource.append('<li value="' + itemValue + '"><i class="la la-arrows"></i> ' + itemText + '</li>')

                // Remove the item from the current source area
                $(this).remove()

                console.warn('hola', itemValue, itemText)
            })
        ...
        function selectAll() {
            ...
            $(theField).find('[data-identifier=drag-source] li').each(function (e) {
                itemValue = $(this).val()
                itemText = $(this).text()

                // Copy the item in the destination area
                $dragDestination.append('<li value="' + itemValue + '"><i class="la la-arrows"></i> ' + itemText + '</li>');

                // Add the item to the select result
                $hiddenSelect.append('<option value="' + itemValue + '" selected></option>');
