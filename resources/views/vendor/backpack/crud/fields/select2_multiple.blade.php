        ...
        @if (isset($field['model']))
            @foreach ($field['options'] as $option)
                @if( (old(square_brackets_to_dots($field["name"])) && in_array($option->getKey(), old($field["name"]))) || (is_null(old(square_brackets_to_dots($field["name"]))) && isset($field['value']) && in_array($option->getKey(), $field['value']->pluck($option->getKeyName(), $option->getKeyName())->toArray())))
                    <option value="{{ $option->getKey() }}" selected>{{ ((isset($field['has_translation']) && $field['has_translation'])?$option->translation->{$field['attribute']}:$option->{$field['attribute']}) }}</option>
                @else
                    <option value="{{ $option->getKey() }}">{{ ((isset($field['has_translation']) && $field['has_translation'])?$option->translation->{$field['attribute']}:$option->{$field['attribute']}) }}</option>
                @endif
            @endforeach
        @endif
    </select>
