...
@section('content')
    <div class="row">
                ...
                <div class="card padding-10">
                    ...
                    <div class="card-footer">
                        <button type="submit" class="btn btn-success">
                            <i class="la la-save"></i> {{ trans('backpack::base.save') }}</button>
        ...
        <div class="col-lg-8">
            <form class="form" action="{{ route('admin.update_locale') }}" method="post">
                    ...
                    <div class="card-body backpack-profile-form bold-labels">
                        <div class="row">
                            <div class="col-md-6 form-group">
                                ...
                                @endphp
                                <label class="required">{{ $label }}</label>
                                <select class="form-control form-control-lg" id="select2" name="{{ $field }}">
                                    @foreach (languages() as $key => $item)
                                        @php
                                            $value = ($key =='ar_TN') ? 'ar' : $key;
                                        @endphp
                                        <option value="{{$value}}" {{old($field) ? (old($field) === $value ? 'selected' : ''): (optional(json_decode($user->settings))->$field === $value ? 'selected' : '')}}>{{ $item }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
        ...
        <div class="col-lg-8">
            <form class="form" action="{{ route('backpack.account.password') }}" method="post">
                ...
                <div class="card padding-10">
                    ...
                    <div class="card-footer">
                        <button type="submit" class="btn btn-success">
                            <i class="la la-save"></i> {{ trans('backpack::base.change_password') }}</button>
                        <a href="{{ backpack_url() }}" class="btn">{{ trans('backpack::base.cancel') }}</a>
                    </div>
