...
<select name="{{ $field['name'] }}@if (isset($field['allows_multiple']) && $field['allows_multiple'] == true)[]@endif" style="width: 100%" id="{{ $field['name'] }}"
    data-init-function="bpFieldInitSelect2FromArrayElement"
    data-field-is-inline="{{ var_export($inlineCreate ?? false) }}"
    data-language="{{ str_replace('_', '-', app()->getLocale()) }}" @include('crud::fields.inc.attributes',
    ['default_class'=> 'form-control select2_from_array'])
    ...
    @push('crud_fields_scripts')
        ...
        <script>
            $(function() {
                ...
                numberForLatest(value);

                $('#select_type').on('change', function() {
                    numberForLatest($(this).val());
                });
            });

            function numberForLatest(value) {
                $('#number_for_latest').parent('div').hide();

                if (value == 'latest') {
                    $('#number_for_latest').parent('div').show();
                }
            }
        </script>
    @endpush
