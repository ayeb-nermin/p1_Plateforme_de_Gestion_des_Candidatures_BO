...
@if ($crud->fieldTypeNotLoaded($field))
    ...
    <script>
        $(function() {
            var value = $('#module_reference').val();
            orderColumn(value);

            $('#module_reference').on('change', function() {
                orderColumn($(this).val());
            });
        });
        ...
        function orderColumn(value) {
            ...
            $.ajax({
                url: "{{ route('getWidgetColumnsByModuleReference') }}",
                data: {
                    reference: value
                }
            }).done(function(response) {
                $.each(response, function(value, text) {
                    $('#order_column').append($('<option></option>', {
                        text: text,
                        value: value
                    }));
                });
